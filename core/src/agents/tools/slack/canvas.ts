...
const canvasBlockSchema = z.object({
  ...
  }).optional().describe('Text content for the block')
}).passthrough();
...
const canvasCriteriaSchema = z.object({
...
export const createEditChannelCanvasTool = (
  ...
  new DynamicStructuredTool({
    ...
    schema: z.object({
      ...
      changes: z.array(canvasChangeSchema).describe('The changes to apply to the canvas. Each change should include properties like title or initial_blocks.'),
    }),
...
export const canvasSectionsLookupTool = (
  ...
  new DynamicStructuredTool({
    ...
    schema: z.object({
      ...
      criteria: canvasCriteriaSchema.describe('The criteria to lookup sections by, such as section_id or blocks_limit.'),
    }),
