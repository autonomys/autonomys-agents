...
const blockSchema = z.object({
  type: z.string().describe('The type of block: "section", "divider", "image", "header", or "context"'),
  text: z.object({
    type: z.string().describe('The type of text: "mrkdwn" for markdown formatting or "plain_text" for plain text'),
    text: z.string().describe('The actual text content to display')
  }).optional().describe('Text content for section, header, or context blocks'),
  // Image block properties
  image_url: z.string().optional().describe('URL to the image (required for image blocks)'),
  alt_text: z.string().optional().describe('Alternative text for the image (required for image blocks)'),
  ...
  }).optional().describe('Optional accessory for section blocks'),
  ...
  elements: z.array(
    z.union([
      ...
      z.object({
        type: z.union([z.literal('mrkdwn'), z.literal('plain_text')]).describe('Text element type'),
        text: z.string().describe('The text content')
      })
    ])
  ).optional().describe('Elements for context blocks')
}).passthrough();
