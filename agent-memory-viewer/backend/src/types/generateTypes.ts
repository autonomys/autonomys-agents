...
async function generateTypes() {
  try {
    ...
    for (const file of schemaFiles) {
      ...
      const unionType = `export type ${versionTypeName} = ${types
        .map(t => t.match(/export interface (\w+)/)?.[1])
        .filter(Boolean)
        .join(' | ')};`;
...
`;
    ...
    await fs.writeFile(path.join(__dirname, '../types/generated/index.ts'), indexContent);
