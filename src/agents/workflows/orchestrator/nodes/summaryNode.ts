...
export const createSummaryNode = (config: OrchestratorConfig) => {
  ...
  return async (state: { messages: readonly BaseMessage[] }) => {
    ...
    const newMessages = recentMessages.map(msg => `${msg.getType()}: ${msg.content}`).join('\n');

    const newSummary = await config.prompts.summaryPrompt.pipe(config.orchestratorModel).invoke({
