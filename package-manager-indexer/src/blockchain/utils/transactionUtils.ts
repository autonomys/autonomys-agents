...
import { retryOperation, withTimeout } from './retryUtils.js';
      ...
      try {
        ...
        if (
          ...
          if (typeof name === 'string' && name.trim() !== '') {
            logger.info(
              `Successfully extracted tool name "${name}" from ${decoded.name} transaction`,
            );
            return name;
          }
        }
      ...
      try {
        ...
        if (decoded.name === 'registerTool' && decoded.args.length >= 4) {
          ...
          if (metadataHash && typeof metadataHash === 'string') {
            logger.info(
              `Successfully extracted metadataHash "${metadataHash}" from registerTool transaction`,
            );
            return metadataHash;
          }
        }
        ...
        if (decoded.name === 'updateToolMetadata' && decoded.args.length >= 4) {
          ...
          if (metadataHash && typeof metadataHash === 'string') {
            logger.info(
              `Successfully extracted metadataHash "${metadataHash}" from updateToolMetadata transaction`,
            );
            return metadataHash;
          }
        }
        ...
        throw new Error(
          `Failed to extract metadataHash from transaction ${txHash}: ${error.message}`,
        );
      }
    },
    3,
