...
///////////KOL///////////
export async function addKOL(kol: { id: string; username: string; created_at?: Date }): Promise<void> {
  const db = await initializeDatabase();
...
export async function updateResponseStatus(id: string, status: 'approved' | 'rejected') {
  const db = await initializeDatabase();
...
export async function addTweet(tweet: {
  ...
  return db.run(
    ...
    `,
    [tweet.id, tweet.text, tweet.user_id, tweet.username, tweet.time_parsed]
  );
}
...
///////////SKIPPED TWEET///////////
export async function addSkippedTweet(skipped: { id: string; tweetId: string; reason: string; confidence: number }) {
  const db = await initializeDatabase();
  return db.run(
    `
...
export async function flagBackSkippedTweet(skippedId: string, reason: string): Promise<boolean> {
  const db = await initializeDatabase();
  const result = await db.run(`UPDATE skipped_tweets SET recheck = FALSE, reason = ? WHERE id = ?`, [
    reason,
    skippedId,
  ]);
  return result !== undefined;
...
///////////DSN///////////
export async function addDsn(dsn: { id: string; tweetId: string; cid: string }) {
  return db?.run(
    `
